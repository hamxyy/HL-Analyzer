...
/**
 * This action is used to show the type information for any program element.
 * It is more or less boilerplate code. The only interesting part is
...
public class ShowTypeDeclarationActionHandler extends AbstractHandler {
	...
	private final class ShowTypeDeclarationAction extends Action {
		...
		private String getDescription(final int offset, final XtextResource resource) {
			// the next couple of lines extract the current semantic model element
			// as opposed to the concrete syntax note.
			IParseResult parseResult = resource.getParseResult();
			ICompositeNode rootNode = parseResult.getRootNode();
			INode currentNode = NodeModelUtils.findLeafNodeAtOffset(rootNode, offset);
			EObject semanticObject = NodeModelUtils.findActualSemanticObjectFor(currentNode);
			// now we assemble the text to be shown in the pop-up.
			// we use the type system  to calculate the type of the object
			// as well as the calculation trace
