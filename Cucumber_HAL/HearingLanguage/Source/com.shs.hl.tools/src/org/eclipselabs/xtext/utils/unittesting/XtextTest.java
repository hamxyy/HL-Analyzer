...
public abstract class XtextTest {
	...
	protected FluentIssueCollection testFile(String fileToTest,
	...
	protected FluentIssueCollection testFileNoSerializer(String fileToTest,
			String... referencedResources) {
	...
	private List<SyntaxErrorMessage> testParserRule(String textToParse,
			String ruleName, boolean errorsExpected) {

		ParserRule parserRule = (ParserRule) GrammarUtil.findRuleForName(
				grammar.getGrammar(), ruleName);
	...
	protected void testParserRuleErrors(String textToParse, String ruleName,
		...
		List<SyntaxErrorMessage> errors = testParserRule(textToParse, ruleName,
				true);
	...
	protected String loadFileContents(String rootPath, String filename) {
		...
		try {
			InputStream is = resourceSet.getURIConverter().createInputStream(
					uri);
	...
	@SuppressWarnings("unchecked")
	protected <T extends EObject> T loadModel(ResourceSet rs, URI uri,
			Class<T> clazz) {
	...
	protected void ignoreUnassertedWarnings() {
		...
		assertedIssues.addAll(issues.warningsOnly().except(assertedIssues)
				.getIssues());
	}
	...
	protected void assertConstraints(String constraintID,
			FluentIssueCollection coll) {
		ensureIsAfterTestFile();
