...
public class StateSpaceHashCodeHelper {
	...
	/**
	 * Map for storing local hash codes of objects. This map computes local hash
	 * codes on demand.
	 */
	...
	private final boolean			graphEquality;

	// Whether to ignore node IDs.
	@SuppressWarnings("unused")
	private final boolean			ignoreNodeIDs;
	...
	private Map<EObject, Integer>	map;
	...
	public int hashCode(EObject model) {
		return hashCode(model, null);
	}

	/**
	 * Compute the hash code for a given model and update the hash-code map.
	 * 
	 * @generated NOT
	 */
	@SuppressWarnings("unchecked")
	public int hashCode(EObject model, @SuppressWarnings("rawtypes") Map map) {
		...
		// Compute the hash-code: downwards
		int result = totalHashCode(null, EcoreUtil2.eAllContentsAsList(model), 0);

		// self
		result += model.hashCode() + findActualNodeFor(model).getStartLine() * 23;
	...
	/*
	 * Compute the total hash code of a list of EObjects. This delegates to
	 * #totalhashCode() for a single EObject.
	 */
	protected int totalHashCode(EObject container, List<EObject> nodes, int depth) {
	...
	/*
	 * Compute the total hash code of a single node. This computes the
	 * context-aware hash code of the current node and merges it with the ones
	 * from the contents of the object. Hence, the method walks down the
	 * containment tree of the node.
	 */
	@SuppressWarnings("unchecked")
	...
	/*
	 * Compute the local hash code for a node. This is done based on the type of
	 * the node, its attribute values and the number of references to other
	 * objects.
	 */
	protected int localHashCode(EObject object) {
		...
		// Use node IDs:
		// if (!ignoreNodeIDs) {
		// List children = EcoreUtil2.getAllContentsOfType(model,
		// object.getClass());
		// int id = EcoreUtil2.getID((EObject)
		// children.get(children.indexOf(object)));
	...
	/*
	 * Compute the context-aware hash code for a node. This combines the local
	 * hash code of the node and its neighbors into a single hash code.
	 */
	@SuppressWarnings("unchecked")
	...
	/*
	 * Combine a list of hash codes into one single hash code. Depending on the
	 * equality type, the list is treated as a sequence or as a set.
	 */
